...
export const Chennai = () => {
    ...
    useEffect(() => {
        fetch("http://localhost:3009/getcheall")
        .then(res => res.json())
    ...
    const del = (id) => {
        var key = { id: id };
        axios.post('http://localhost:3009/deleteche', key)
        .then((response, error) => {
            if (error) {
 ...
 <section id="c" style={{ display: 'flex', flexWrap: 'wrap', margin: '60px', gap: '50px', justifyContent: 'center' }}>
    ...
    <Link to={`/getchennai/${value.id}`} style={{ fontWeight: 'bold', fontStyle: 'italic', color: 'blue', textDecoration: 'none' }}>View more<span>&rarr;</span></Link>
</div>
