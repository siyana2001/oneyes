...
export const Bangall = () => {

    const [data, setData] = useState([]);

    useEffect(() => {
        fetch("http://localhost:3009/getbangall")
        .then(res => res.json())
    ...
    const del = (id) => {
        var key = { id: id };
        axios.post('http://localhost:3009/deletebang', key)
        .then((response, error) => {
            if (error) {
                    ...
                    <Card style={{ height: '100%', width: '100%', position: 'relative' }}>
                        <Card.Body style={{ padding: '20px' }}>
                            ...
                            <div style={{ textAlign: 'left' }}>
                                ...
                                <p style={{fontStyle:'oblique'}}>CTC: <BsCurrencyRupee/>{value.ctc} -/year</p>
                                <p style={{fontStyle:'italic'}}> <BsTools /> Skills: {value.skills}</p>
                            </div>
                            <div style={{ position: 'absolute', bottom: '10px', right: '15px' }}>
    <Link to={`/getbang/${value.id}`} style={{ fontWeight: 'bold', fontStyle: 'italic', color: 'blue', textDecoration: 'none' }}>View more<span>&rarr;</span></Link>
</div>
