...
import { BsHouse, BsHouseDashFill, BsCurrencyRupee, BsTools,BsGeoAlt } from 'react-icons/bs';
...
const Delget = () => {
    ...
    const del = (id) => {
        var key = { id: id };
        axios.post('http://localhost:3009/delete', key)
        .then((response, error) => {
            if (error) {
    ...
    <Link to={`/getone/${value.id}`} style={{ fontWeight: 'bold', fontStyle: 'italic', color: 'blue', textDecoration: 'none' }}>View more<span>&rarr;</span></Link>
</div>
