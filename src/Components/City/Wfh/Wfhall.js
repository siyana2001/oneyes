...
import { BsHouse, BsHouseDashFill, BsCurrencyRupee, BsTools } from 'react-icons/bs';
...
export const Wfhall = () => {
    ...
    useEffect(() => {
        fetch("http://localhost:3009/getwfhall")
            .then(res => res.json())
    ...
    const del = (id) => {
        var key = { id: id };
        axios.post('http://localhost:3009/deletewfh', key)
            .then((response, error) => {
                if (error) {
    ...
    return (
        <>
        <Navbar/>
             <Link to="/student" style={{ position: 'absolute', top: '30px', left: '20px', zIndex: '1000', marginTop:"40px", fontWeight: 'bold', textDecoration: 'none', color: 'blue' }}>
                        ...
                        <Card style={{ height: '100%', width: '100%', position: 'relative' }}>
                            <Card.Body style={{ padding: '20px' }}>
                                ...
                                <div style={{ textAlign: 'left' }}>
                                    <p><BsHouseDashFill /> {value.location}</p>
                                ...
                                <div style={{ position: 'absolute', bottom: '10px', right: '15px' }}>
                                    <Link to={`/getwfh/${value.id}`} style={{ fontWeight: 'bold', fontStyle: 'italic', color: 'blue', textDecoration: 'none' }}>View more<span>&rarr;</span></Link>
                                </div>
                            </Card.Body>
                        </Card>
                    </div>
